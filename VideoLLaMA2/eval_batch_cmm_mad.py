...
sys.path.append('./')
from videollama2 import model_init, mm_infer, mm_logit, mm_contrast_decode
...
def run_inference(model, processor, tokenizer, sample, modal_type, accelerator):
    ...
    audio_path = video_path.replace('.mp4', '.wav')  # Assuming audio file has same name with .wav extension
    ...
    try:
        ...
        question_with_prompt = question + " Answer only 'Yes' or 'No'. Do not include any explanation."
...
def evaluate_dataset_accelerate(args):
    ...
    if accelerator.is_main_process:
        ...
        with open(output_path.replace('.json', f'_{args.modal_type}_{args.gamma}.json'), 'w', encoding='utf-8') as f:
