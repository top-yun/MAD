...
def run_inference(model, processor, sample, modal_type, accelerator, args):
    ...
    try:
        ...
        if modal_type == "a":
            audio_path = sample.get('audio_full_path')
            if not audio_path:
                raise ValueError("Audio path missing for audio-only evaluation.")
            content = [{"type": "audio", "audio": audio_path}]
        elif modal_type == "v":
            video_path = sample.get('video_full_path')
            if not video_path:
                raise ValueError("Video path missing for video evaluation.")
            content = [{"type": "video", "video": video_path}]
        else:  # modal_type == "av"
            video_path = sample.get('video_full_path')
            if not video_path:
                raise ValueError("Video path missing for audio-visual evaluation.")
            content = [{"type": "video", "video": video_path}]

        prompted_question = question + " Answer only 'Yes' or 'No'. Do not include any explanation."
        content.append({"type": "text", "text": prompted_question})
        ...
        #process_audio_info : audios , process_video_info : images, videos
...
def evaluate_dataset_qwen_omni(args):
    ...
    if accelerator.is_main_process:
        ...
        output_file = output_path.replace('.json', f'_{args.modal_type}.json')
